...
from app.services.documentos_service import validar_reglas_cronologicas_documento
...
def _parse_date_yyyy_mm_dd(v: str) -> date:
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="fecha_documento inválida. Formato esperado: YYYY-MM-DD."
        )
...
# UPLOAD
# ====================================================
async def upload_batch_documento_service(
    db: Session,
    *,
    doc_id: int,
    fecha_documento: str,
    ...
    # CAMBIO 🔥 traer datos reales necesarios
    q = text("""
        SELECT id, tipo_doc_id, batch_id
    ...
    # CAMBIO 🔥 VALIDACIÓN ANTES DE TODO
    validar_reglas_cronologicas_documento(
        db,
    ...
    # subir a MinIO SOLO si pasó validación
    try:
...
# DOWNLOAD
# ====================================================
def download_batch_documento_service(
    db: Session,
    *,
    doc_id: int,
):
